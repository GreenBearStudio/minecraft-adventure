import os
import requests
import shutil
from tusclient import client

# CONFIG
ACCESS_TOKEN = os.environ.get("VIMEO_TOKEN", "YOUR_VIMEO_ACCESS_TOKEN")
SOURCE_FOLDER = "todo"
DEST_FOLDER = "uploaded"
OUTPUT_FILE = "uploaded_video_links.txt"

VIMEO_UPLOAD_ENDPOINT = "https://api.vimeo.com/me/videos"

def create_upload_ticket(file_path):
    """Create an upload ticket with Vimeo API and return upload URL + video URI."""
    headers = {
        "Authorization": f"Bearer {ACCESS_TOKEN}",
        "Content-Type": "application/json",
        "Accept": "application/vnd.vimeo.*+json;version=3.4"
    }
    data = {
        "upload": {
            "approach": "tus",
            "size": os.path.getsize(file_path)
        },
        "name": os.path.basename(file_path),
        "privacy": {"view": "anybody"}  # public link
    }
    response = requests.post(VIMEO_UPLOAD_ENDPOINT, headers=headers, json=data)
    response.raise_for_status()
    video_data = response.json()
    upload_link = video_data["upload"]["upload_link"]
    video_uri = video_data["uri"]
    return upload_link, video_uri

def upload_video(file_path):
    """Upload video using tuspy and return Vimeo share link."""
    upload_link, video_uri = create_upload_ticket(file_path)

    tus_client = client.TusClient(upload_link, headers={
        "Authorization": f"Bearer {ACCESS_TOKEN}"
    })
    uploader = tus_client.uploader(file_path=file_path, chunk_size=5242880)  # 5MB chunks
    uploader.upload()  # handles resume and chunked transfer

    video_id = video_uri.split("/")[-1]
    share_link = f"https://player.vimeo.com/video/{video_id}"
    return share_link

def main():
    if not os.path.exists(DEST_FOLDER):
        os.makedirs(DEST_FOLDER)

    with open(OUTPUT_FILE, "a") as out_file:
        for filename in os.listdir(SOURCE_FOLDER):
            file_path = os.path.join(SOURCE_FOLDER, filename)
            if os.path.isfile(file_path) and filename.lower().endswith((".mp4", ".mov", ".avi")):
                print(f"Uploading {filename}...")
                try:
                    share_link = upload_video(file_path)
                    out_file.write(f"{filename}# {share_link}\n")
                    print(f"Uploaded {filename} → {share_link}")

                    # Move file to uploaded folder
                    shutil.move(file_path, os.path.join(DEST_FOLDER, filename))
                except Exception as e:
                    print(f"Failed to upload {filename}: {e}")

if __name__ == "__main__":
    main()

